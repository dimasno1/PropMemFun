...
- (void)viewDidLoad {
    ...
    _sushiTypes = [[NSArray alloc] initWithObjects: @"California Roll", //After that retainCount = 1
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString* sushiName = [_sushiTypes objectAtIndex: indexPath.row]; // No need to release, autorelease is pending
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString* message = [[[NSString alloc] initWithFormat: @"Last selected: %@:, Current: %@", _lastSushiSelected, sushiName] autorelease];
    ...
    [_lastSushiSelected release];
    _lastSushiSelected = nil;
    _lastSushiSelected = [sushiName retain];
}
